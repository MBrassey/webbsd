#!/usr/bin/env python3
"""Replace the 3rd urxvt in startup.sh with tty-clock."""
...
# Line 18 is the 3rd "i3-msg 'exec urxvt'" - replace it
print("\n=== Replacing line 18 (T3) with clock ===")
send_cmd(f"sed -i '' '18s|exec urxvt|exec urxvt -e tty-clock -c -C 1 -t|' {HOME}/.config/i3/startup.sh")

# Verify
serial_buf = b""
send(f"sed -n '16,20p' {HOME}/.config/i3/startup.sh\n", 2)
time.sleep(3); drain()
print("Lines 16-20:")
for line in serial_buf.decode(errors="replace").split("\n"):
    s = line.strip()
    if s and not s.startswith("$") and not s.startswith("sed "):
        print(f"  {s}")
