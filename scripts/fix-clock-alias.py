#!/usr/bin/env python3
"""Add 'clock' alias for tty-clock in fish shell config."""
...
# Add fish alias
print("\n=== Adding clock alias to fish config ===")
send_cmd(f"mkdir -p {HOME}/.config/fish/functions")
send(f"rm -f {HOME}/.config/fish/functions/clock.fish\n", 0.3)
send(f"echo 'function clock' >> {HOME}/.config/fish/functions/clock.fish\n", 0.04)
send(f"echo '    tty-clock -c -C 1 -t' >> {HOME}/.config/fish/functions/clock.fish\n", 0.04)
send(f"echo 'end' >> {HOME}/.config/fish/functions/clock.fish\n", 0.04)
time.sleep(1); drain()

# Verify
serial_buf = b""
send(f"cat {HOME}/.config/fish/functions/clock.fish\n", 2)
time.sleep(2); drain()
print("clock.fish:")
