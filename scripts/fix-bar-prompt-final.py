#!/usr/bin/env python3
"""Fix: clickable i3bar buttons (proper architecture), Nerd Font icons,
delayed clear in terminals so it runs AFTER i3 resize."""
...
# Status bar: click reader in BACKGROUND, output loop in FOREGROUND
# This prevents stdout interleaving (only main loop writes stdout)
STATUS_SH_LINES = [
    '#!/bin/sh',
    ...
    'echo \'[]\'',
    '',
    '# Click event reader in BACKGROUND (reads stdin, never writes stdout)',
    '(while read line; do',
    '    case "$line" in',
    '        *"bsd"*)',
    ...
    'done) &',
    '',
    '# Main output loop in FOREGROUND (sole stdout writer)',
...
# Golden-3term: use delayed clear so it runs AFTER resize operations
# Terminal B's resize happens ~20s in. Using "sleep 12; clear" in fish -C
# means clear runs ~12s after fish init (which takes ~2-3s) = ~15s after launch.
# Terminal B launches at ~10s, so clear at ~25s. Resize at ~20s. Clear is AFTER resize.
GOLDEN_3TERM_LINES = [
    '#!/bin/sh',
...
print("=== Fix bar buttons + prompt position ===")
...
# 1. Write status.sh
print("\n=== Writing status.sh (click reader bg, output fg) ===")
write_lines(STATUS_SH_LINES, f"{HOME}/.config/i3/status.sh", executable=True)

# 2. Write golden-3term.sh with delayed clear
print("\n=== Writing golden-3term.sh (delayed clear) ===")
write_lines(GOLDEN_3TERM_LINES, f"{HOME}/.config/i3/golden-3term.sh", executable=True)

# 3. Bump bar height for bigger icons
print("\n=== Updating i3 bar height ===")
send_cmd(f"sed -i '' 's|height 22|height 26|' {I3CFG}")

# 4. Fix i3 terminal binding
send_cmd(f"sed -i '' 's|exec urxvt.*|exec urxvt -e fish -C clear|' {I3CFG}")
