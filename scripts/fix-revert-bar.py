#!/usr/bin/env python3
"""Revert bar to working printf approach (no click_events),
fix terminal prompt position with fish -C clear."""
...
# Proven working status.sh (printf approach, no click_events)
# FreeBSD icon U+F30C = octal 357 214 214
STATUS_SH_LINES = [
    '#!/bin/sh',
    ...
    '    dt=$(date \'+%a %b %d  %H:%M\')',
    "    printf ',['",
    '    printf \'{"full_text":" %s ","color":"#ab1100","separator":false,"separator_block_width":12},\' "$BSD"',
...
# Golden-3term: use "fish -C clear" so clear runs AFTER all init + resize
GOLDEN_3TERM_LINES = [
    '#!/bin/sh',
...
# 1. Revert status.sh to working printf approach
print("\n=== Writing status.sh (printf, no click_events) ===")
write_lines(STATUS_SH_LINES, f"{HOME}/.config/i3/status.sh", executable=True)

# 2. Fix golden-3term.sh with fish -C clear
print("\n=== Writing golden-3term.sh (fish -C clear) ===")
write_lines(GOLDEN_3TERM_LINES, f"{HOME}/.config/i3/golden-3term.sh", executable=True)

# 3. Fix i3 config terminal binding to also use fish -C clear
print("\n=== Fixing i3 terminal binding ===")
send_cmd(f"sed -i '' 's|exec urxvt -e fish$|exec urxvt -e fish -C clear|' {I3CFG}")
send_cmd(f"sed -i '' 's|exec urxvt -e fish |exec urxvt -e fish -C clear |' {I3CFG}")
...
print("\n=== Verifying ===")
serial_buf = b""
send(f"grep 'fish.*clear' {I3CFG}\n", 1)
send(f"grep 'fish.*clear' {HOME}/.config/i3/golden-3term.sh | head -1\n", 1)
send(f"head -3 {HOME}/.config/i3/status.sh\n", 1)
time.sleep(2)
drain()
for line in serial_buf.decode(errors="replace").split("\n"):
    s = line.strip()
    if s and ("fish" in s or "status" in s or "#!/" in s) and not s.startswith("$"):
        print(f"  {s}")
