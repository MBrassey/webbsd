#!/usr/bin/env python3
"""Configure guest networking for wsproxy relay (wss://relay.widgetry.org/).
The relay provides real DHCP/DNS, so remove all hardcoded DNS config
and force DHCP renewal on startup."""
...
print("=== Configure networking for wsproxy relay ===")
...
# 1. Remove hardcoded DNS — let the relay's DHCP provide DNS
print("\n=== Removing hardcoded DNS config ===")
send_cmd("rm -f /etc/dhclient.conf")
# Set a temporary resolv.conf (relay will override via DHCP)
send_cmd("echo 'nameserver 8.8.8.8' > /etc/resolv.conf")

# 2. Ensure rc.conf uses DHCP for vtnet0
send_cmd("sed -i '' '/ifconfig_vtnet0/d' /etc/rc.conf")
send_cmd("echo 'ifconfig_vtnet0=\"DHCP\"' >> /etc/rc.conf")

# 3. Update net-watchdog: force DHCP renewal on startup, then monitor
print("\n=== Writing net-watchdog ===")
watchdog = [
    '#!/bin/sh',
    '# Network watchdog for wsproxy relay',
    '# Force DHCP renewal on startup (state restore changes network)',
    '# Then monitor and re-DHCP if IP lost',
    '',
    '# Initial: release old lease and get new one from relay',
    'sleep 2',
...
serial_buf = b""
send("cat /etc/dhclient.conf 2>&1\n", 1)
time.sleep(1)
drain()
print(f"\ndhclient.conf: {serial_buf.decode(errors='replace').strip()}")
